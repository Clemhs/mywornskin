    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-zinc-950/95 backdrop-blur-lg border-b border-zinc-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
            ...
            {['FR', 'EN', 'ES', 'DE'].map((lang) => (
              <button
                ...
                className={`px-3.5 py-1.5 text-xs font-medium rounded-full transition ${
                  language === lang ? 'bg-rose-600 text-white' : 'hover:bg-zinc-800'
                }`}
              >
                {lang}
              </button>
          ...
          <Link 
            href="/messages"
            className="bg-rose-600 hover:bg-rose-500 px-5 py-2.5 rounded-2xl text-sm font-semibold transition flex items-center gap-2 whitespace-nowrap"
          >
            Messages
          </Link>
        </div>
