...
export default function CreatorsClient({ initialCreators }: { initialCreators: any[] }) {
  ...
  const filteredCreators = useMemo(() => {
    return initialCreators.filter(creator => {
      ...
      if (activeFilter === 'new') matchesFilter = new Date(creator.created_at) > new Date(Date.now() - 30*24*60*60*1000);

      return matchesSearch && matchesCountry && matchesCity && matchesSize && matchesShoeSize && matchesFilter;
    });
  }, [initialCreators, searchTerm, selectedCountry, selectedCity, selectedSize, selectedShoeSize, activeFilter]);

  const countries = [...new Set(initialCreators.map(c => c.country).filter(Boolean))];
  const cities = [...new Set(initialCreators.map(c => c.city).filter(Boolean))];
  const sizes = [...new Set(initialCreators.map(c => c.size).filter(Boolean))];
  const shoeSizes = [...new Set(initialCreators.map(c => c.shoe_size).filter(Boolean))];
