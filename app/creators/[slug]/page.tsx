...
export default function CreatorProfile() {
  ...
  useEffect(() => {
    ...
    const fetchCreator = async () => {
      const { data, error } = await supabase
        .from('profiles')
        .select('*')           // ← Simple, sans join pour l'instant
        .eq('username', slug)
        .single();
  ...
  if (loading) return <div className="min-h-screen bg-zinc-950 text-white pt-20 flex items-center justify-center text-xl">Chargement...</div>;
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="pt-20 max-w-6xl mx-auto px-6">
        <div className="h-80 bg-gradient-to-r from-rose-950 to-zinc-900 rounded-3xl relative mb-12 overflow-hidden">
          {/* Bannière future */}
        </div>
