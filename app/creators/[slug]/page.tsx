...
export default function CreatorProfile() {
  ...
  const fetchCreatorData = async () => {
    ...
    try {
      ...
      const { data: creatorData, error: creatorError } = await supabase
        .from('profiles') // ou 'creators' selon ta table
        .select('*')
        .eq('username', slug)
        .eq('role', 'creator')
        .single();
  ...
  if (error || !creator) {
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
        <p className="text-xl text-red-400">{error || 'Créatrice non trouvée'}</p>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-16 relative z-10">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="pt-6 flex-1">
            ...
            <p className="text-zinc-400 mt-2">{creator.bio || "Aucune bio pour le moment."}</p>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {(creator.badges || ["Voie Sensuelle", "Chasseuse d'Odeurs"]).map((badge: string, i: number) => (
                <div 
                  key={i}
                  className="flex items-center gap-2 bg-zinc-900 border border-zinc-700 px-6 py-3 rounded-2xl text-sm font-medium"
                >
        ...
        {reviews.length > 0 && (
          <div className="mt-16">
            <h2 className="text-3xl font-light mb-8">Avis clients ({reviews.length})</h2>
