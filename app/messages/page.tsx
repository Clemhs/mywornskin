    ...
    <div className="flex h-screen bg-zinc-950 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-4 border-b border-zinc-800 flex items-center gap-4 bg-zinc-900">
          <button onClick={() => setShowSidebar(true)} className="md:hidden text-3xl pr-4">☰</button>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-zinc-950">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
              <div className={`chat-bubble max-w-[80%] md:max-w-[65%] ${msg.sender === 'me' ? 'chat-bubble-sent' : 'chat-bubble-received'}`}>
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-900">
          ...
          <div className="flex gap-3">
            <button onClick={() => fileInputRef.current?.click()} className="w-12 h-12 flex items-center justify-center bg-zinc-800 hover:bg-zinc-700 rounded-2xl text-2xl transition">📷</button>
            
            <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="w-12 h-12 flex items-center justify-center bg-zinc-800 hover:bg-zinc-700 rounded-2xl text-2xl transition">😀</button>
            ...
            <button onClick={sendMessage} className="bg-rose-600 hover:bg-rose-500 px-8 py-4 rounded-3xl font-medium transition">Envoyer</button>
          </div>
