...
export default function Verify() {
  ...
  const submitVerification = () => {
    ...
    setTimeout(() => {
      setStatus('submitted');
      setLoading(false);
      alert("✅ Documents envoyés ! Votre profil sera vérifié dans les 48h par l'administrateur.");
    }, 1400);
  };
    ...
    <div className="min-h-screen bg-zinc-950 py-12">
      <div className="max-w-2xl mx-auto px-6">
        <div className="text-center mb-12">
          ...
          <p className="text-zinc-400">Cette étape est obligatoire pour apparaître sur le site.</p>
        </div>

        <div className="card p-10">
          <div className="mb-10">
            <h3 className="font-semibold mb-3">1. Selfie de vérification</h3>
            <p className="text-sm text-zinc-400 mb-4">Photo de vous tenant une feuille avec écrit "MyWornSkin" + date du jour.</p>
            <label className="block border-2 border-dashed border-zinc-700 hover:border-rose-500 rounded-3xl p-12 text-center cursor-pointer">
              ...
              {selfie ? <img src={selfie} alt="selfie" className="mx-auto max-h-64 rounded-2xl" /> : <div className="text-6xl mb-4">📸</div>}
            </label>
          </div>
          ...
          <div className="mb-12">
            ...
            <label className="block border-2 border-dashed border-zinc-700 hover:border-rose-500 rounded-3xl p-12 text-center cursor-pointer">
              ...
              {idCard ? <img src={idCard} alt="id" className="mx-auto max-h-64 rounded-2xl" /> : <div className="text-6xl mb-4">🪪</div>}
            </label>
          </div>
          ...
          {status === 'submitted' && (
            <div className="mt-8 text-center text-emerald-400">
              Documents envoyés avec succès.<br />Vous serez notifié dès validation.
            </div>
          )}
        </div>
      </div>
