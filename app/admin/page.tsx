...
export default function AdminPage() {
  ...
  const handlePhotoAction = async (
    ...
    if (action === 'approved' && profile[pendingField]) {
      // Copier l'URL pending vers l'URL définitive
      await supabase
        .from('profiles')
        .update({
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-8">
      <div className="max-w-6xl mx-auto">
            ...
            {pendingPhotos.length === 0 ? (
              ...
              pendingPhotos.map((p) => (
                <div key={p.id} className="bg-zinc-900 rounded-3xl p-8">
                  <h3 className="font-semibold text-xl mb-6">@{p.username || p.full_name}</h3>
        ...
        {/* Reviews tab à compléter plus tard si besoin */}
      </div>
    </div>
  );
}
