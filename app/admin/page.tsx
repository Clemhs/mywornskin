      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex border-b border-zinc-800 mb-10">
          <button 
            ...
            className={`px-8 py-4 font-medium flex items-center gap-3 transition-all ${activeTab === 'photos' ? 'border-b-4 border-pink-500 text-white' : 'text-zinc-400 hover:text-white'}`}
          >
            <ImageIcon size={22} /> Photos en attente
          </button>
          <button 
            onClick={() => setActiveTab('reviews')} 
            className={`px-8 py-4 font-medium flex items-center gap-3 transition-all ${activeTab === 'reviews' ? 'border-b-4 border-pink-500 text-white' : 'text-zinc-400 hover:text-white'}`}
          >
            <AlertTriangle size={22} /> Commentaires refusés
          </button>
          <button 
            onClick={() => setActiveTab('messages')} 
            className={`px-8 py-4 font-medium flex items-center gap-3 transition-all ${activeTab === 'messages' ? 'border-b-4 border-pink-500 text-white' : 'text-zinc-400 hover:text-white'}`}
          >
        ...
        {activeTab === 'photos' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {pendingPhotos.length === 0 ? (
              <p className="text-zinc-500 text-xl col-span-full">Aucune photo en attente de validation pour le moment.</p>
            ) : (
              pendingPhotos.map((p) => (
        ...
        {activeTab === 'reviews' && <div className="text-zinc-400">Commentaires refusés (à compléter)</div>}
        {activeTab === 'messages' && <div className="text-zinc-400">Messages Admin (à compléter)</div>}
      </div>
    </div>
  );
}
